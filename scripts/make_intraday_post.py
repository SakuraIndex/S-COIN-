...
def read_intraday(csv_path: str, index_key: str) -> pd.DataFrame:
    """
    CSVを読み込み、Datetime列をUTC→JSTに変換してDatetimeIndexに設定。
    """
...
def compute_intraday_change(anchor_value: float, last_value: float, value_type: str) -> float:
    """
    騰落率計算:
      - percent: 列がすでに％値の場合 → 差分 (last - anchor)
      - raw:     通常の価格の場合 → (last/anchor - 1)*100
    """
    if value_type == "percent":
        change_pct = last_value - anchor_value
    else:
        change_pct = (last_value / anchor_value - 1.0) * 100
    return round(float(change_pct), 3)


def plot_snapshot(df_jst: pd.DataFrame, index_key: str, out_png: str, session_start: str, session_end: str):
    """
    黒背景・白枠なしのチャートを生成。
    """
    plt.style.use("dark_background")
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(df_jst.index, df_jst[index_key], color="cyan", linewidth=2)
